...
# Chatbot CHELSEA: CHat Emotion Logic SEnse Automator (0.21) (BETA)
...
############################################################################################################
# Version 0.19 (Allowed CHELSEA to use current topics to search for relevant 'why is/are' questions): 10-31-2024
# Version 0.20 (Now works experimentally with bigrams using a topic or depth word as a starter to 'imagine' a response): 11-08-2024
# Version 0.21 (Now can use trigrams in same kind of way): 11-09-2024
	...
	def use_imagination(self):
		...
		if ((ngram_count > 1000 and ngram_count < 25,000 and random.randint(1, 9) == 1) or (ngram_count > 30,000 and random.randint(1, 7) == 1)):
			if (len(self.trigram_dictionary.keys()) > 0 and random.randint(1, 3) == 1):
				...
				self.Xchatlog.append(f"{self.bot_name} (Thinking): Trigram chain in {direction_type} for {word_type} word '{word}' imagined.")
