...
#Now includes code for building trigram dictionary!
...
for emotion in nEmotions:
    ...
    for message in messages:
         message_words = (re.sub(r"([^a-z0-9 '\-])", '', message)).split(" ")
         for n in range(len(message_words)):
              if n != 0 and n != len(message_words) - 1:
                    try:
                        trigram_dictionary[f"{message_words[n-1]} {message_words[n]} {message_words[n+1]}"]["seen"] += 1
                    except(KeyError):
                        try:
                            trigram_dictionary[f"{message_words[n-1]} {message_words[n]} {message_words[n+1]}"] = {"seen": 1, "emotions": [dictionary[message_words[n-1]]["emotion"], dictionary[message_words[n]]["emotion"], dictionary[message_words[n+1]]["emotion"]]}
                        except(KeyError):
                             continue

with open("trigramDictionary.json", 'w') as trigram_dictionary_file:
     json.dump(trigram_dictionary, trigram_dictionary_file, indent=4)