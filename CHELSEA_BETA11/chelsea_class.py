...
# Chatbot CHELSEA: CHat Emotion Logic SEnse Automator (0.16) (BETA)
...
class chelsea:
	def __init__(self, bot_name):
		...
		self.topics = {}
		self.previous_pairs = []
		self.unanswered_question_asked = False
	...
	def output_chelsea_data(self):
		...
		data_file.write(f"\nNumber of unanswered questions: {len(self.unanswered_questions.keys())}")
		data_file.close()
	...
	def check_for_answer(self):
		...
		if (self.unanswered_question_asked):
			...
			if (question_word):
				...
				if (answer):
					...
					self.Xchatlog.append(f"{self.bot_name} (Thinking): Deleted unanswered what is/are question.")
					del self.unanswered_questions[temp_question]
				else:
					self.Xchatlog.append(f"{self.bot_name} (Thinking): Unanswered question still not answered, moving on.")		
		self.unanswered_question_asked = False
		...
		if (random.randint(1, 8) == 1 and len(list(self.unanswered_questions.keys())) > 0):
			...
			for word in self.message_words:
				try:
					...
					self.unanswered_question_asked = True
					response_made = True
					break
				except(KeyError):
					continue
			if response_made:
				return
			#No unanswered question match found from previous user reply words, use random question instead
			self.Xchatlog.append(f"{self.bot_name} (Thinking): Asked random what is/are question, waiting for valid answer.")
			self.CHELSEA_previous_response = self.botReply(random.choice(list(self.unanswered_questions.keys())))
			self.unanswered_question_asked = True
		else:
